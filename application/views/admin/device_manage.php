    ...
    <div class="row">
      <div class="col-sm-12">
        <div class="card-box table-responsive">
          <button onclick="show_modal('<?php echo base_url() . 'index.php?admin/popup_modal/device_add' . "'";?>)" class="btn btn-success waves-effect waves-light"><i class="fa fa-plus"></i> Tambah Equipment</button>
          <button onclick="show_modal('<?php echo base_url() . 'index.php?admin/popup_modal/category_add' . "'";?>)" class="btn btn-success waves-effect waves-light"><i class="fa fa-plus"></i> Tambah Kategori</button>
          ...
          foreach ($categories as $category):
            ...
            ?>
              <div class="tab-pane <?php if($cat_sl==1){ echo "active";} ?>" id="<?php echo 'cat_'.$category['category_id'];?>">
                <table id="datatable-buttons1<?php echo $category['category_id'];?>" class="table table-striped table-success">
                  <thead>
                    <tr>
                      <th>#</th>
                      <th>Nama Barang</th>
              ...
              ?>
                    <tr id='<?php echo $device['equipment_id'];?>'>
                      <td><?php echo $sl++;?></td>
                      <td><strong><?php echo $device['equipment_name'];?></strong></td>
                      <td><?php echo $device['purchase_date'];?></td>
                      <td><?php echo $device['no_spk'];?></td>
                      <td><?php echo $device['no_referensi'];?></td>
                      <td><?php echo $this->common_model->get_brand_name($device['brand_id']);?></td>
                      <td><?php echo $device['model'];?></td>
                      <td><?php echo $device['issue_to'];?></td>
                      <td><div class="btn-group m-b-20"> <a title="Details" class="btn btn-icon waves-effect waves-light btn-info btn-xs" onclick="show_modal('<?php echo base_url() . 'index.php?admin/popup_modal/device_detail/'. $device['equipment_id'] . "'";?>)"><i class="fa fa-info-circle" aria-hidden="true"></i></a> <a title="Copy" class="btn btn-icon waves-effect waves-light btn-success btn-xs" onclick="show_modal('<?php echo base_url() . 'index.php?admin/popup_modal/device_copy/'. $device['equipment_id'] . "'";?>)"><i class="fa fa-files-o" aria-hidden="true"></i></a> <a title="Edit" class="btn btn-icon waves-effect waves-light btn-inverse btn-xs" onclick="show_modal('<?php echo base_url() . 'index.php?admin/popup_modal/device_edit/'. $device['equipment_id'] . "'";?>)"><i class="fa fa-pencil"></i></a> <a title="Delete" class="btn btn-icon waves-effect waves-light btn-danger btn-xs" onclick="delete_device(<?php echo $device['equipment_id'];?>)" href="#" class="delete"><i class="fa fa-remove"></i></a> </div></td>
                    </tr>
                    <?php endforeach;?>
                  </tbody>
                </table>
